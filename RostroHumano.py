...
def detectar_rostro(imagen):
    ...
    if len(rostros) > 0:
        print("Se ha detectado un rostro humano.")
        return True, rostros
    else:
        print("No se ha detectado ningún rostro humano.")
        return False, []
...
def main():
    ...
    while True:
        ...
        if rostro_detectado:
            ...
            cv2.putText(frame, "Se ha detectado un rostro humano", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            # Convertir el texto a voz y reproducirlo
            texto_a_voz("Se ha detectado un rostro humano")
            reproducir_audio()
        else:
            equis=1
